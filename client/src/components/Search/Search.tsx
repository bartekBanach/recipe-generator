...
const Search = ({selected, setSelected, setOffset}:SearchProps) => {
    ...
    const filteredIngredients = useMemo(() => {
        ...
        if (query==='') return [];
        else return ingredients.filter(item => item.name.toLowerCase().includes(query.toLowerCase()));
        ...
        <>
            <form className={styles.form}>
                <div className={styles.searchbar}>
                    ...
                    {filteredIngredients.length !== 0 && <ul className={styles.autocomplete}>
                        {filteredIngredients.slice(0,15).map(item => (
                            <li 
                                key={item.id} 
                                className={`${styles.option} ${item.id === highlighted && styles.highlighted}`} 
                                onClick={() => handleClick(item)}
                                onMouseEnter={() => setHighlighted(item.id)}
                                >
                                    {item.name}
                                    {selected.includes(item) ? <FaCircleMinus /> : <FaCirclePlus />}
                            </li>
                        ))}
                    </ul>}
                </div>
